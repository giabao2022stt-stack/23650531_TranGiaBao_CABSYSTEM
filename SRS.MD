...
Xây dựng mô hình dữ liệu dựa trên các Business Requirement, Functional Requirement và Business Rule đã xác định.

Mô hình dữ liệu tập trung vào quy trình chính:

**Customer → Booking → Driver Matching → Trip → Fare → Payment → Rating**

---

## 2. Xác định các Entity

| STT | Entity | Vai trò |
|---:|---|---|
| 1 | **User** | Quản lý tài khoản và thông tin xác thực |
| 2 | **Customer** | Quản lý thông tin khách hàng |
| 3 | **Driver** | Quản lý thông tin và trạng thái tài xế |
| 4 | **Vehicle** | Quản lý phương tiện của tài xế |
| 5 | **VehicleType** | Quản lý loại xe/dịch vụ |
| 6 | **Booking** | Lưu yêu cầu đặt xe của khách hàng |
| 7 | **DriverAssignment** | Lưu quá trình tìm và phân công tài xế |
| 8 | **Trip** | Quản lý chuyến đi thực tế |
| 9 | **Location** | Lưu vị trí của tài xế |
| 10 | **Fare** | Quản lý thông tin tính cước |
| 11 | **Payment** | Quản lý giao dịch thanh toán |
| 12 | **Rating** | Quản lý đánh giá tài xế |
| 13 | **Notification** | Quản lý thông báo |
...
# 4. Giải thích các quan hệ chính
...
`DriverAssignment` giúp lưu lại lịch sử quá trình tìm tài xế.

---

## 4.3. Booking – Trip

Một Booking thành công sẽ tạo ra một Trip.
...
Driver quản lý phương tiện được sử dụng để thực hiện chuyến.
...
## 4.5. VehicleType – Vehicle

Một loại xe có thể được sử dụng bởi nhiều Vehicle.
...
Một Trip có thể phát sinh nhiều Payment Record để hỗ trợ Retry Payment.
...
Một Driver có thể gửi nhiều dữ liệu vị trí trong quá trình hoạt động.

```text
Driver 1 ──────── N Location
```

Dữ liệu Location hỗ trợ:
...
> **Lưu ý:** Các trạng thái `CANCELLED`, `TIMEOUT`, `NO_DRIVER_AVAILABLE` và cách chuyển trạng thái cần được xác nhận chính thức với khách hàng.
...
flowchart TD
    ...
    C["Lọc Driver Available"]
    D["Kiểm tra loại xe"]
    E["Xếp hạng Driver"]
    F["Gửi Booking cho Driver"]
    G{"Driver phản hồi?"}
    H["Accept"]
    I["Reject"]
    J["Timeout"]
    K["Gán Driver"]
    L["Tìm Driver tiếp theo"]
    M{"Còn Driver?"}
    N["Thông báo Customer"]
...
# 7. Nguyên tắc thiết kế Data Model

| Nguyên tắc | Áp dụng |
|---|---|
| **Separation of Concerns** | Tách Customer, Driver, Booking, Trip, Payment thành các Entity riêng |
| **Traceability** | Có thể truy vết Booking → Assignment → Trip → Payment |
| **Auditability** | Lưu AuditLog cho các thao tác quan trọng |
| **Security** | Không lưu trực tiếp thông tin thanh toán nhạy cảm |
| **Scalability** | Location và Notification được tách riêng |
| **Extensibility** | VehicleType hỗ trợ bổ sung loại dịch vụ |
| **Payment Retry** | Payment tách khỏi Trip để hỗ trợ nhiều lần thanh toán |
| **Driver Matching** | DriverAssignment lưu lịch sử Accept / Reject / Timeout |

---

# 8. Tổng quan Data Flow
...
Mô hình dữ liệu của CAB System được xây dựng xoay quanh các Entity chính:

> **User → Customer / Driver → Vehicle → Booking → DriverAssignment → Trip → Fare → Payment → Rating**

Các Entity hỗ trợ gồm:

> **Location → Notification → AuditLog**

Mô hình ERD này là **Logical Data Model**, được sử dụng làm cơ sở cho các bước tiếp theo:
...
> **Kết quả Bước 9:** Xác định được các Entity, thuộc tính chính, Primary Key, Foreign Key và quan hệ giữa các Entity phục vụ toàn bộ vòng đời đặt xe.

bước 10: xác định non functional requirement
BƯỚC 10 – XÁC ĐỊNH NON-FUNCTIONAL REQUIREMENTS
1. Mục tiêu
Xác định các Non-Functional Requirements (NFR) nhằm đảm bảo CAB System hoạt động nhanh, ổn định, bảo mật, có khả năng mở rộng và dễ bảo trì.

2. Danh sách Non-Functional Requirements
Mã	Nhóm	Tên yêu cầu	Mô tả	Mức độ
NFR01	Performance	Hiệu năng	Hệ thống phải phản hồi nhanh đối với các chức năng chính.	Critical
NFR02	Performance	Driver Matching	Hệ thống phải tìm và phân công tài xế trong thời gian phù hợp.	Critical
NFR03	Scalability	Khả năng mở rộng	Có thể mở rộng khi số lượng khách hàng, tài xế và chuyến đi tăng.	Critical
NFR04	Availability	Tính sẵn sàng	Hệ thống phải hoạt động ổn định, đặc biệt trong giờ cao điểm.	Critical
NFR05	Reliability	Độ tin cậy	Lỗi Payment hoặc Notification không được làm dừng toàn bộ hệ thống.	Critical
NFR06	Security	Bảo mật	Bảo vệ dữ liệu cá nhân, phương tiện, vị trí và giao dịch.	Critical
NFR07	Authentication	Xác thực	Người dùng phải được xác thực trước khi sử dụng chức năng yêu cầu tài khoản.	High
NFR08	Authorization	Phân quyền	Kiểm soát quyền truy cập đối với các chức năng quản trị.	Critical
NFR09	Data Privacy	Bảo mật dữ liệu thanh toán	Không lưu trực tiếp thông tin thẻ/tài khoản thanh toán nhạy cảm.	Critical
NFR10	Audit	Lưu vết	Ghi nhận các thao tác quan trọng để phục vụ kiểm tra và xử lý sự cố.	High
NFR11	Maintainability	Khả năng bảo trì	Dễ sửa lỗi, nâng cấp và thay đổi từng module.	High
NFR12	Extensibility	Khả năng mở rộng chức năng	Có thể thêm dịch vụ, Payment Provider và Notification Provider mới.	High
NFR13	Deployability	Khả năng triển khai	Có thể triển khai chức năng mới từng phần mà hạn chế ảnh hưởng hệ thống.	High
NFR14	Fault Tolerance	Khả năng chịu lỗi	Có cơ chế xử lý khi Payment, Notification, GPS hoặc Network gặp lỗi.	Critical
NFR15	Data Integrity	Toàn vẹn dữ liệu	Đảm bảo dữ liệu Booking, Trip, Fare và Payment chính xác, nhất quán.	Critical
NFR16	Monitoring	Giám sát	Theo dõi trạng thái, hiệu năng và lỗi của hệ thống.	High
NFR17	Backup & Recovery	Sao lưu và phục hồi	Dữ liệu quan trọng phải được backup và có khả năng phục hồi.	High

3. Các yêu cầu NFR trọng yếu
NFR01 – Performance
Hệ thống phải đảm bảo thời gian phản hồi phù hợp đối với:
...
Cập nhật trạng thái Trip.
Thanh toán.
Cần xác nhận: thời gian phản hồi tối đa và SLA cụ thể với khách hàng.

NFR02 – Driver Matching Performance
Driver Matching là chức năng quan trọng của hệ thống.
...
Mục tiêu là giảm thời gian khách hàng phải chờ tài xế.

NFR03 – Scalability
Hệ thống phải có khả năng mở rộng khi:

Số lượng Customer tăng.
Số lượng Driver tăng.
Số lượng Booking tăng.
Số lượng cập nhật Location tăng.
Lưu lượng Notification tăng.
Các module có tải cao cần có khả năng scale độc lập.

NFR04 – Availability
CAB System phải hoạt động ổn định trong thời gian nhu cầu cao.

Peak Hour
    ↓
Traffic tăng
    ↓
System vẫn hoạt động
    ↓
Booking không bị gián đoạn

NFR05 – Reliability & Fault Tolerance
Lỗi của một thành phần không được làm ảnh hưởng toàn bộ hệ thống.
...
Cho phép Retry
        ↓
Booking/Trip vẫn hoạt động

Tương tự với Notification Provider:

Notification lỗi
        ↓
Retry / Queue
        ↓
Không làm dừng Booking
...
Location Data.
Transaction Data.
NFR07 – Authentication
Các đối tượng phải được xác thực trước khi sử dụng chức năng yêu cầu tài khoản:
...
NFR09 – Payment Data Privacy
CAB System không lưu trực tiếp:

Số thẻ.
CVV.
Mật khẩu ngân hàng.
Thông tin thanh toán nhạy cảm.
Thông tin thanh toán được xử lý thông qua Payment Provider.
...
Hệ thống cần được thiết kế theo hướng module hóa:
...
Thay đổi một module không nên gây ảnh hưởng lớn đến các module khác.
...
SMS/Email/Push Channel mới.
7. Deployability
NFR13 – Independent Deployment
...
Mục tiêu là hạn chế ảnh hưởng đến các chức năng đang hoạt động.

8. Data Integrity
NFR15 – Data Integrity
Dữ liệu phải đảm bảo:

Chính xác.
Nhất quán.
Không bị trùng lặp.
Không bị mất trong quá trình xử lý.
Đặc biệt đối với:
...
Fare.
Payment.
Ví dụ: một Booking không được đồng thời được hai Driver nhận thành công.
...
System Availability.
10. Backup & Recovery
NFR17 – Backup & Recovery
Dữ liệu quan trọng cần được backup và có khả năng phục hồi:
...
Payment.
Audit Log.
11. Ma trận ưu tiên NFR
Priority	Nhóm NFR
🔴 Critical	Performance
🔴 Critical	Driver Matching
...
12. Các NFR cần xác nhận với khách hàng
STT	Nội dung cần xác nhận
1	Response Time tối đa là bao nhiêu giây?
2	Driver Matching phải hoàn thành trong bao lâu?
3	Số lượng Customer dự kiến?
4	Số lượng Driver dự kiến?
5	Số Booking tối đa trong giờ cao điểm?
6	SLA/Availability yêu cầu bao nhiêu %?
7	RTO – thời gian phục hồi hệ thống?
8	RPO – lượng dữ liệu tối đa có thể mất?
9	Thời gian lưu trữ dữ liệu?
10	Thời gian lưu Location History?
11	Chính sách Retry Payment/Notification?
12	Yêu cầu bảo mật và Compliance cụ thể?

13. Kết luận
Các Non-Functional Requirements trọng yếu của CAB System:

Performance → Scalability → Availability → Reliability → Security → Data Integrity → Maintainability → Extensibility

Đây là cơ sở để chuyển sang các bước tiếp theo:
...
Solution Architecture / System Design
        ↓
Development & Testing
bước 11 và 12 : xác định và vẽ các usecase kí hiệu UC, use case customer thì ghi là UC01 customer , đặc tả use case
ChatGPT đã nói:
BƯỚC 11 & 12 – XÁC ĐỊNH VÀ ĐẶC TẢ USE CASE
1. Mục tiêu
Xác định các Use Case (UC) chính của CAB System và đặc tả các luồng nghiệp vụ quan trọng dựa trên Business Requirement và Functional Requirement.

BƯỚC 11 – XÁC ĐỊNH USE CASE
2. Danh sách Use Case
Mã	Tên Use Case	Actor chính	Mô tả
UC01	Customer – Quản lý tài khoản	Customer	Đăng ký, đăng nhập, cập nhật thông tin cá nhân
UC02	Customer – Tạo Booking	Customer	Nhập điểm đón, điểm đến, chọn loại xe và tạo yêu cầu
UC03	Customer – Theo dõi Trip	Customer	Theo dõi tài xế, ETA và trạng thái chuyến
UC04	Customer – Thanh toán	Customer	Thanh toán bằng tiền mặt hoặc trực tuyến
UC05	Customer – Xem lịch sử Trip	Customer	Xem lịch sử chuyến và cước
UC06	Customer – Đánh giá Driver	Customer	Đánh giá tài xế sau chuyến
UC07	Driver – Quản lý hồ sơ	Driver	Cập nhật thông tin cá nhân và phương tiện
UC08	Driver – Quản lý trạng thái	Driver	Online/Offline/Available
UC09	Driver – Nhận/Từ chối Booking	Driver	Accept hoặc Reject yêu cầu chuyến
UC10	Driver – Cập nhật Trip	Driver	Cập nhật trạng thái chuyến
UC11	Driver – Cập nhật vị trí	Driver	Gửi vị trí hiện tại
UC12	Driver Matching	System	Tìm và phân công tài xế
UC13	Fare Calculation	System	Tính cước chuyến
UC14	Notification	System	Gửi thông báo cho Customer/Driver
UC15	Quản lý Customer	Operations	Quản lý khách hàng
UC16	Quản lý Driver	Operations	Quản lý tài xế
UC17	Quản lý Vehicle	Operations	Quản lý phương tiện
UC18	Quản lý Trip	Operations	Theo dõi và xử lý chuyến
UC19	Tra cứu Transaction	Operations/Finance	Tra cứu giao dịch
UC20	Reporting	Operations/Management	Báo cáo hoạt động
UC21	Authorization Management	Administrator	Quản lý quyền
UC22	Audit Log	Administrator	Theo dõi thao tác quan trọng

3. Use Case Diagram
Code dưới đây có thể copy trực tiếp vào file .md trên GitHub.
```mermaid
...
flowchart LR
    ...
    Location["Location / Map Provider"]

    subgraph CAB["CAB SYSTEM"]

        UC01["UC01<br/>Quản lý tài khoản"]
        UC02["UC02<br/>Tạo Booking"]
        UC03["UC03<br/>Theo dõi Trip"]
        UC04["UC04<br/>Thanh toán"]
        UC05["UC05<br/>Lịch sử Trip"]
        UC06["UC06<br/>Đánh giá Driver"]

        UC07["UC07<br/>Quản lý hồ sơ Driver"]
        UC08["UC08<br/>Quản lý trạng thái Driver"]
        UC09["UC09<br/>Accept / Reject Booking"]
        UC10["UC10<br/>Cập nhật Trip"]
        UC11["UC11<br/>Cập nhật vị trí"]

        UC12["UC12<br/>Driver Matching"]
        UC13["UC13<br/>Fare Calculation"]
        UC14["UC14<br/>Notification"]

        UC15["UC15<br/>Quản lý Customer"]
        UC16["UC16<br/>Quản lý Driver"]
        UC17["UC17<br/>Quản lý Vehicle"]
        UC18["UC18<br/>Quản lý Trip"]
        UC19["UC19<br/>Tra cứu Transaction"]
        UC20["UC20<br/>Reporting"]

        UC21["UC21<br/>Authorization"]
        UC22["UC22<br/>Audit Log"]
    end
...
4. UC01 – Quản lý tài khoản Customer
Thành phần	Nội dung
Use Case	UC01
Tên	Quản lý tài khoản
Actor	Customer
Mục tiêu	Đăng ký, đăng nhập và cập nhật thông tin
Pre-condition	Customer chưa đăng nhập hoặc đã có tài khoản
Trigger	Customer thực hiện chức năng tài khoản
Post-condition	Tài khoản được tạo/cập nhật thành công
...
Trigger	Customer chọn đặt xe
Post-condition	Booking được tạo và chuyển sang tìm Driver
...
Hệ thống tạo Booking
   ↓
UC12 - Driver Matching

Exception
Điểm đón không hợp lệ.
Điểm đến không hợp lệ.
Không xác định được vị trí.
Không có loại xe phù hợp.
Mất kết nối mạng.
6. UC12 – Driver Matching
Thành phần	Nội dung
Use Case	UC12
Tên	Driver Matching
Actor	System
Actor phụ	Driver, Location Provider
Mục tiêu	Tìm và phân công Driver phù hợp
Pre-condition	Booking đã được tạo
Trigger	Booking chuyển sang SEARCHING_DRIVER
Post-condition	Driver được phân công hoặc thông báo không tìm được Driver
...
Tìm Driver tiếp theo
Còn Driver phù hợp?
Thông báo CustomerKhông tìm được Driver
Business Rules
Chỉ Driver đang Available mới được Matching.
Driver phải phù hợp với loại xe/dịch vụ.
Ưu tiên Driver gần Customer và phù hợp tiêu chí vận hành.
Driver Reject → tìm Driver tiếp theo.
Driver Timeout → tìm Driver tiếp theo.
Không có Driver → thông báo Customer.
Customer không cần tạo lại Booking.
7. UC09 – Driver Accept / Reject Booking
Thành phần	Nội dung
Use Case	UC09
Tên	Nhận/Từ chối Booking
Actor	Driver
Pre-condition	Driver Available và nhận được Booking
Trigger	Driver nhận thông báo chuyến
Post-condition	Booking được Accept hoặc chuyển sang Driver khác
...
Tìm Driver tiếp theo
Còn Driver?
Thông báo CustomerKhông có Driver
8. UC10 – Driver cập nhật Trip
Trip State
stateDiagram-v2
    [*] --> DRIVER_ASSIGNED
    DRIVER_ASSIGNED --> DRIVER_ARRIVING
...
CANCELLED	Đã hủy
FAILED	Chuyến bị lỗi

9. UC13 – Fare Calculation
Thành phần	Nội dung
Use Case	UC13
Tên	Tính cước
Actor	System
Pre-condition	Trip đã hoàn thành
Trigger	Trip chuyển sang COMPLETED
Post-condition	Fare được tính và lưu
...
Công thức tính cước, giá tối thiểu và phụ phí cần được xác nhận với khách hàng.
...
Phương thức	Cash / Electronic Payment
Pre-condition	Fare đã được xác định
Post-condition	Payment được ghi nhận
...
11. UC03 – Theo dõi Trip
Thành phần	Nội dung
Use Case	UC03
Tên	Theo dõi Trip
Actor	Customer
Mục tiêu	Theo dõi tài xế và trạng thái chuyến
Thông tin	Driver, Vehicle, Location, ETA, Trip Status
...
Post-condition	Rating được lưu

Main Flow
Trip COMPLETED
      ↓
Customer chọn Rating
      ↓
Nhập điểm đánh giá
      ↓
Nhập nhận xét
...
13. UC18 – Operations quản lý Trip
Thành phần	Nội dung
Use Case	UC18
Tên	Quản lý Trip
Actor	Operations Staff
Mục tiêu	Theo dõi và xử lý Trip
Pre-condition	Operations đã đăng nhập
...
Trip Report	Tổng số chuyến
Revenue Report	Doanh thu
Completion Report	Tỷ lệ hoàn thành
Cancellation Report	Tỷ lệ hủy
Driver Performance	Hiệu quả Driver
Payment Report	Kết quả thanh toán

15. Luồng Use Case tổng thể
UC01Quản lý tài khoản
UC02Tạo Booking
UC12Driver Matching
UC09Driver Accept / Reject
UC10Thực hiện Trip
UC03Theo dõi Trip
UC13Fare Calculation
UC04Payment
UC06Rating
UC14Notification
16. Ưu tiên Use Case cho MVP
Priority	Use Case	Mức độ
P1	UC01 – Quản lý tài khoản	Critical
P1	UC02 – Tạo Booking	Critical
P1	UC12 – Driver Matching	Critical
P1	UC09 – Accept/Reject Booking	Critical
P1	UC10 – Thực hiện Trip	Critical
P1	UC03 – Theo dõi Trip	Critical
P1	UC13 – Fare Calculation	Critical
P1	UC04 – Payment	Critical
P1	UC14 – Notification	Critical
P2	UC06 – Rating	High
P2	UC15–UC19 – Operations	High
P2	UC20 – Reporting	High
P2	UC21 – Authorization	High
P2	UC22 – Audit Log	High

Kết luận
Bước 11: Xác định Actor và danh sách Use Case của CAB System.

Bước 12: Đặc tả Use Case theo cấu trúc:
...
Luồng nghiệp vụ cốt lõi của CAB:
...
Bộ Use Case này là cơ sở để chuyển sang Acceptance Criteria → Test Case → Solution Design → Development.
Bước 13: Acceptance Criteria những tiêu chí chấp nhận, mục đích của , xác định khi nào kết thúc và nghiệm thu,làm theo dạng markdow, code mermaid
ChatGPT đã nói:
BƯỚC 13 – XÁC ĐỊNH ACCEPTANCE CRITERIA
1. Mục tiêu
Acceptance Criteria (AC) là các tiêu chí dùng để xác định một yêu cầu đã được thực hiện đúng và đủ hay chưa.

Acceptance Criteria giúp:

Xác định rõ khi nào chức năng được xem là hoàn thành.
Là cơ sở để khách hàng nghiệm thu.
Là cơ sở để QA/Tester xây dựng Test Case.
Tránh hiểu khác nhau giữa Business – BA – Developer – QA.
Xác nhận Business Requirement và Functional Requirement đã được đáp ứng.
2. Nguyên tắc xác định Acceptance Criteria
Một Acceptance Criteria tốt phải:

Tiêu chí	Mô tả
Clear	Rõ ràng, không gây hiểu nhầm
Testable	Có thể kiểm thử
Measurable	Có thể đo lường khi cần
Complete	Bao phủ Main Flow và Exception Flow
Business-oriented	Phản ánh đúng mục tiêu nghiệp vụ
Independent	Có thể xác nhận độc lập

3. Acceptance Criteria chính của CAB System
Mã AC	Use Case	Acceptance Criteria	Kết quả nghiệm thu
AC01	UC01 – Customer Account	Customer có thể đăng ký, đăng nhập và cập nhật thông tin cá nhân hợp lệ	Pass khi các chức năng hoạt động đúng
AC02	UC02 – Create Booking	Customer nhập điểm đón, điểm đến, loại xe và tạo Booking thành công	Booking được tạo và chuyển sang trạng thái tìm Driver
AC03	UC12 – Driver Matching	Hệ thống tự động tìm Driver phù hợp dựa trên trạng thái, vị trí và loại xe	Driver phù hợp được tìm thấy
AC04	UC12 – Driver Matching	Nếu Driver từ chối hoặc hết thời gian phản hồi, hệ thống phải tìm Driver tiếp theo	Không yêu cầu Customer tạo lại Booking
AC05	UC12 – Driver Matching	Nếu không còn Driver phù hợp, hệ thống phải thông báo cho Customer	Customer nhận được thông báo
AC06	UC09 – Accept/Reject	Driver Available có thể nhận hoặc từ chối Booking	Booking được xử lý đúng trạng thái
AC07	UC10 – Trip	Driver có thể cập nhật các trạng thái của Trip theo đúng quy trình	Trạng thái Trip được cập nhật chính xác
AC08	UC03 – Tracking	Customer có thể theo dõi Driver, vị trí và trạng thái Trip	Thông tin được cập nhật
AC09	UC13 – Fare	Hệ thống tính đúng Fare theo Business Rule đã được xác nhận	Fare được lưu và hiển thị chính xác
AC10	UC04 – Payment	Customer có thể thanh toán bằng tiền mặt hoặc thanh toán điện tử	Payment được ghi nhận đúng trạng thái
AC11	UC04 – Payment	Khi thanh toán điện tử thất bại, hệ thống thông báo và xử lý Retry theo chính sách	Không làm dừng toàn bộ hệ thống
AC12	UC14 – Notification	Customer/Driver nhận được thông báo tại các sự kiện quan trọng	Notification được gửi thành công
AC13	UC06 – Rating	Customer chỉ được đánh giá Driver sau khi Trip hoàn thành	Rating được lưu hợp lệ
AC14	UC18 – Operations	Operations có thể theo dõi và xử lý Trip	Dữ liệu hiển thị chính xác
AC15	UC20 – Reporting	Hệ thống cung cấp các báo cáo theo yêu cầu	Báo cáo được tạo chính xác
AC16	UC21 – Authorization	User chỉ được thực hiện chức năng đúng với Role/Permission	Truy cập trái quyền bị từ chối
AC17	UC22 – Audit	Các thao tác quản trị quan trọng được ghi nhận Audit Log	Có thể truy vết User, Action, Time, Result

4. Acceptance Criteria cho quy trình đặt xe
Đây là Acceptance Criteria quan trọng nhất của CAB System.
...
Quy trình được xem là đạt khi:

Customer tạo được Booking hợp lệ.
Hệ thống tự động tìm Driver.
Chỉ Driver đủ điều kiện mới được đề xuất.
Driver Accept → Booking được Assign.
Driver Reject → hệ thống tìm Driver tiếp theo.
Driver Timeout → hệ thống tìm Driver tiếp theo.
Không còn Driver → Customer được thông báo.
Customer không phải tạo lại Booking khi Driver Reject/Timeout.
5. Acceptance Criteria cho thanh toán
Cash
Electronic
Success
Failed
...
Retry theo chính sách
Điều kiện nghiệm thu
Customer có thể chọn Cash hoặc Electronic Payment.
Cash Payment được ghi nhận thành công.
Electronic Payment được xử lý thông qua Payment Provider.
Payment Success → ghi nhận giao dịch thành công.
Payment Failed → thông báo Customer.
Payment Failed không làm dừng Booking/Trip.
CAB không lưu trực tiếp thông tin thẻ hoặc thông tin thanh toán nhạy cảm.
6. Acceptance Criteria cho Security & Authorization
Authorized
Unauthorized
User
Authentication
...
Điều kiện nghiệm thu
Mã	Điều kiện
AC-S01	User chưa đăng nhập không được truy cập chức năng yêu cầu authentication
AC-S02	Customer chỉ được truy cập chức năng của Customer
AC-S03	Driver chỉ được truy cập chức năng của Driver
AC-S04	Operations chỉ được thực hiện chức năng được cấp quyền
AC-S05	Administrator được phép thực hiện các chức năng quản trị theo Permission
AC-S06	Thao tác quản trị quan trọng phải được ghi Audit Log

7. Definition of Done – Điều kiện kết thúc
Một Use Case/Requirement chỉ được xem là hoàn thành và sẵn sàng nghiệm thu khi:
...
Điều kiện DONE
 ...
 Functional Requirement đã được triển khai.
 Main Flow hoạt động đúng.
 Exception/Alternative Flow được xử lý.
 Acceptance Criteria đạt.
 QA/Tester xác nhận.
 Không còn lỗi Critical/Blocker.
 Customer/Business xác nhận nghiệm thu.
8. Các tiêu chí cần xác nhận thêm với khách hàng
Một số Acceptance Criteria hiện chưa thể chốt con số cụ thể vì Business Rule chưa được khách hàng xác nhận:

STT	Nội dung cần xác nhận
1	Thời gian tối đa Driver được phép Accept/Reject
2	Thời gian tối đa cho Driver Matching
3	Response Time của hệ thống
4	Số lần Retry khi Driver không phản hồi
5	Chính sách Retry Payment
6	Chính sách Cancellation
7	SLA/Availability
8	Thời gian cập nhật Location
9	Thời gian lưu dữ liệu
10	Quy tắc tính Fare

Lưu ý: Không nên tự đưa các con số như 3 giây, 10 giây, 99.9% vào Acceptance Criteria khi khách hàng chưa xác nhận. BA cần đưa các giá trị này vào Open Questions để chốt trước khi baseline requirement.

9. Kết luận Bước 13
Acceptance Criteria là cầu nối giữa Requirement → Development → QA → Customer Acceptance.

Quy trình nghiệm thu của CAB:
...
Mục tiêu cuối cùng: xác định rõ hệ thống phải làm gì, làm như thế nào và điều kiện nào để khách hàng xác nhận là đã đạt yêu cầu.
bước 14: requirement truy xuất nguồn gốc yêu cầu , ma trận truy xuất yêu cầu ,cột 1 BG, cột thứ 2 BR cột thứ 3 FR,, cột UC, cột tiếp theo AC, Cột tiếp theo TC
ChatGPT đã nói:
BƯỚC 14 – REQUIREMENT TRACEABILITY MATRIX (RTM)
1. Mục tiêu
Requirement Traceability Matrix (RTM) – Ma trận truy xuất nguồn gốc yêu cầu dùng để theo dõi mối liên hệ từ Business Goal → Business Requirement → Functional Requirement → Use Case → Acceptance Criteria → Test Case.

Mục đích:

Đảm bảo mọi Business Goal (BG) đều được chuyển thành Business Requirement (BR).
Đảm bảo mỗi BR có Functional Requirement (FR) tương ứng.
Đảm bảo yêu cầu được thể hiện trong Use Case (UC).
Mỗi yêu cầu có Acceptance Criteria (AC) để nghiệm thu.
Mỗi AC có Test Case (TC) để kiểm thử.
Phát hiện yêu cầu bị thiếu hoặc chưa được kiểm thử.
2. Chuỗi truy xuất yêu cầu
Business GoalBG
Business RequirementBR
Functional RequirementFR
Use CaseUC
Acceptance CriteriaAC
Test CaseTC
3. Ma trận truy xuất yêu cầu – RTM
BG	BR	FR	UC	AC	TC
BG01 – Giảm thời gian tìm và phân công Driver	BR01 – Giảm thời gian tìm Driver	FR01 – Xác định vị trí Customer	UC02 – Tạo Booking	AC02 – Booking được tạo thành công	TC01 – Kiểm tra tạo Booking
BG01	BR01	FR02 – Lọc Driver Online/Available	UC12 – Driver Matching	AC03 – Tự động tìm Driver phù hợp	TC02 – Kiểm tra tìm Driver
BG01	BR01	FR03 – Ưu tiên Driver phù hợp	UC12 – Driver Matching	AC03 – Driver phù hợp được ưu tiên	TC03 – Kiểm tra ưu tiên Driver
BG02 – Tự động hóa tìm Driver	BR02 – Tự động tìm và phân công Driver	FR04 – Gửi yêu cầu đến Driver	UC12 – Driver Matching	AC04 – Reject/Timeout → tìm Driver tiếp theo	TC04 – Kiểm tra Driver Reject
BG02	BR02	FR05 – Xử lý Driver Timeout	UC09 – Accept/Reject Booking	AC04 – Timeout → tìm Driver tiếp theo	TC05 – Kiểm tra Driver Timeout
BG02	BR02	FR06 – Thông báo khi không tìm được Driver	UC12 – Driver Matching	AC05 – Không còn Driver → thông báo Customer	TC06 – Kiểm tra không có Driver
BG03 – Hỗ trợ thanh toán Cash/Electronic	BR03 – Hỗ trợ nhiều phương thức thanh toán	FR07 – Chọn phương thức thanh toán	UC04 – Payment	AC10 – Customer chọn Cash/Electronic	TC07 – Kiểm tra chọn Payment Method
BG03	BR03	FR08 – Xử lý Cash Payment	UC04 – Payment	AC10 – Cash Payment được ghi nhận	TC08 – Kiểm tra Cash Payment
BG03	BR03	FR09 – Xử lý Electronic Payment	UC04 – Payment	AC10 – Electronic Payment được xử lý	TC09 – Kiểm tra Electronic Payment
BG03	BR03	FR10 – Xử lý Payment Failed	UC04 – Payment	AC11 – Payment Failed → thông báo và Retry	TC10 – Kiểm tra Payment Failed
BG04 – Nâng cao trải nghiệm Customer	BR04 – Cho phép Customer theo dõi Trip	FR11 – Hiển thị thông tin Driver	UC03 – Tracking Trip	AC08 – Customer theo dõi Driver	TC11 – Kiểm tra Tracking
BG04	BR04	FR12 – Cập nhật vị trí Driver	UC03 – Tracking Trip	AC08 – Vị trí được cập nhật	TC12 – Kiểm tra cập nhật Location
BG04	BR04	FR13 – Hiển thị trạng thái Trip	UC03 – Tracking Trip	AC08 – Trip Status được cập nhật	TC13 – Kiểm tra Trip Status
BG04	BR04	FR14 – Đánh giá Driver	UC06 – Rating	AC13 – Chỉ đánh giá sau khi Trip hoàn thành	TC14 – Kiểm tra Rating
BG05 – Nâng cao hiệu quả vận hành	BR05 – Quản lý Customer/Driver/Trip	FR15 – Quản lý Customer	UC15 – Quản lý Customer	AC14 – Operations quản lý dữ liệu	TC15 – Kiểm tra quản lý Customer
BG05	BR05	FR16 – Quản lý Driver	UC16 – Quản lý Driver	AC14 – Operations quản lý Driver	TC16 – Kiểm tra quản lý Driver
BG05	BR05	FR17 – Quản lý Trip	UC18 – Quản lý Trip	AC14 – Operations theo dõi Trip	TC17 – Kiểm tra quản lý Trip
BG05	BR05	FR18 – Báo cáo vận hành	UC20 – Reporting	AC15 – Báo cáo được tạo chính xác	TC18 – Kiểm tra Reporting
BG07 – An toàn và tin cậy	BR07 – Bảo mật và phân quyền	FR19 – Authentication	UC01 – Customer Account	AC-S01 – User chưa đăng nhập bị từ chối	TC19 – Kiểm tra Authentication
BG07	BR07	FR20 – Authorization/RBAC	UC21 – Authorization	AC16 – User chỉ được truy cập đúng quyền	TC20 – Kiểm tra Authorization
BG07	BR07	FR21 – Audit Log	UC22 – Audit Log	AC17 – Ghi nhận thao tác quản trị	TC21 – Kiểm tra Audit Log
BG07	BR07	FR22 – Fault Tolerance	UC04 – Payment	AC11 – Payment lỗi không làm dừng hệ thống	TC22 – Kiểm tra Payment Failure

4. Sơ đồ truy xuất cho quy trình chính
Đối với quy trình đặt xe và tìm Driver, có thể truy xuất như sau:

BG01Giảm thời gian tìm Driver
BR01Giảm thời gian tìm & phân công Driver
FR01Xác định vị trí Customer
FR02Lọc Driver Available
FR03Ưu tiên Driver phù hợp
FR04Gửi yêu cầu Driver
FR05Xử lý Reject / Timeout
FR06Thông báo không tìm được Driver
UC02Tạo Booking
UC12Driver Matching
UC09Driver Accept / Reject
AC02Booking được tạo
AC03Tìm Driver phù hợp
AC04Reject / Timeout → Driver tiếp theo
AC05Không còn Driver → thông báo
TC01Test Create Booking
TC02Test Driver Matching
TC03Test Driver Reject
TC04Test Driver Timeout
TC05Test No Driver
5. Quy tắc truy xuất
Mỗi yêu cầu nên có thể truy xuất hai chiều:

Forward Traceability
Từ mục tiêu nghiệp vụ xuống kiểm thử:
...
Từ Test Case quay ngược về Business Goal:
...
Mục đích là đảm bảo không có Test Case nào kiểm thử một chức năng không có yêu cầu, đồng thời không có yêu cầu nào bị bỏ sót không được kiểm thử.

6. Kiểm tra độ bao phủ yêu cầu
Kiểm tra	Kết quả mong muốn
BG có BR tương ứng?	Có
BR có FR tương ứng?	Có
FR có UC tương ứng?	Có
UC có AC tương ứng?	Có
AC có TC tương ứng?	Có
Có BR nào không có FR?	Không
Có FR nào không có UC?	Không
Có AC nào không có TC?	Không
Có TC nào không truy xuất được về BG?	Không

7. Kết luận Bước 14
Requirement Traceability Matrix của CAB System được xây dựng theo chuỗi:
...
BG: Tại sao doanh nghiệp cần hệ thống?
BR: Doanh nghiệp cần hệ thống đáp ứng điều gì?
FR: Hệ thống phải thực hiện chức năng gì?
UC: Actor tương tác với hệ thống như thế nào?
AC: Khi nào chức năng được xem là đạt?
TC: Làm thế nào để kiểm thử chức năng đó?
Bước 14 hoàn thành khi mọi BG → BR → FR → UC → AC → TC đều truy xuất được với nhau, không có yêu cầu bị bỏ sót và không có Test Case nằm ngoài phạm vi yêu cầu.
